    ...
    async with task_dict_lock:
        for tk in task_dict.values():
            if hasattr(tk, "seeding"):
                await tk.update()
            if tk.gid().startswith(gid) or tk.gid().endswith(gid):
                return tk
        return None
...
def get_readable_time(seconds: int, full_time=False):
    periods = [
...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    for index, task in enumerate(
        ...
        msg += f"╰●🛑 /{BotCommands.CancelTaskCommand[1]}_{short_gid}\n\n"
